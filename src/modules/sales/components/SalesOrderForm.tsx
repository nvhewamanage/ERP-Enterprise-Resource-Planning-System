...
import { createSalesOrderSchema, type CreateSalesOrderSchema } from "../validations/sales-order.schema";
...
export function SalesOrderForm({
  ...
  const {
    ...
    setError,
  } = useForm<CreateSalesOrderSchema>({
    resolver: zodResolver(createSalesOrderSchema),
    defaultValues: { customerId: "", items: [{ productId: "", quantity: 1, unitPrice: 0 }] },
  });
  ...
  async function onSubmit(values: CreateSalesOrderSchema) {
    ...
    reset({ customerId: "", items: [{ productId: "", quantity: 1, unitPrice: 0 }] });
    onCreated();
  }
      ...
      <div>
        ...
        <div className="flex flex-col gap-3">
          {fields.map((field, index) => {
            const selectedProduct = productFor(items?.[index]?.productId ?? "");
            const requestedQty = items?.[index]?.quantity ?? 0;
            const insufficientStock = selectedProduct && requestedQty > selectedProduct.quantityOnHand;
            ...
            return (
              <div key={field.id} className="grid grid-cols-12 items-start gap-2 rounded-md border border-zinc-200 bg-white p-3">
                <div className="col-span-5">
                  ...
                  />
                  {selectedProduct && (
                    <p className={`mt-1 text-xs ${insufficientStock ? "text-rose-600" : "text-zinc-500"}`}>
                      {selectedProduct.quantityOnHand} in stock
                    </p>
                  )}
        ...
        <p className="mt-2 text-xs text-zinc-500">
          Stock is only checked when the order is fulfilled — a draft can be created even if it exceeds what&apos;s on hand right now.
        </p>
      </div>
